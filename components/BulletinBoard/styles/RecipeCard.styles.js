...
const cardPulse = keyframes`
  0% {
    transform: translate3d(var(--x), var(--y), 0) rotate(var(--rotate)) scale(0);
    opacity: 0;
  }
  50% {
...
const pinnedShake = keyframes`
  0%, 100% {
    transform: translate3d(var(--x), var(--y), 0) rotate(var(--rotate));
  }
  25% {
    transform: translate3d(calc(var(--x) - 2px), var(--y), 0) rotate(var(--rotate));
  }
  75% {
    transform: translate3d(calc(var(--x) + 2px), var(--y), 0) rotate(var(--rotate));
  }
...
export const CardContainer = styled.div`
  ...
  transition: ${props => props.$isDragging
    ? 'none'
    : 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1), box-shadow 0.3s cubic-bezier(0.4, 0, 0.2, 1), width 0.3s ease, height 0.3s ease, min-height 0.3s ease'};
  ...
  animation: ${props => getCardAnimation(props)};
  contain: layout style;
  will-change: ${props => (props.$isDragging || props.$isNew || props.$isShaking) ? 'transform' : 'auto'};
...
export const RecipeCardWrapper = styled.div`
  ...
  will-change: ${props => props.$isFlipped ? 'transform' : 'auto'};
`;
