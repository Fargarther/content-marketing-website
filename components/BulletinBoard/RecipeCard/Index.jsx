...
const RecipeCard = ({
  ...
  const toggleExpand = (e) => {
    ...
    if (onExpand) {
      onExpand(card.id, newExpandedState);
    }
  };
  ...
  const handleAddComment = (comment) => {
    if (onAddComment) {
      onAddComment(card.id, card.recipeId, comment);
    }
  };
  ...
  return (
    <CardContainer
      ...
      onMouseDown={onMouseDown ? (e) => onMouseDown(e, card.id) : undefined}
      onTouchStart={onMouseDown ? (e) => onMouseDown(e, card.id) : undefined}
    >
      <PinButton
            ...
            <CardSide>
              <CardFront
                ...
                onRatingChange={(value) => {
                  if (onRatingChange) {
                    onRatingChange(card.id, card.recipeId, value);
                  }
                }}
              />
              <FlipIndicator 
...
const areEqual = (prevProps, nextProps) => {
  ...
  if (prevProps.isShaking !== nextProps.isShaking) return false;
  if (prevProps.rating !== nextProps.rating) return false;
  if (prevProps.boardWidth !== nextProps.boardWidth) return false;
  ...
  if (prevCard.recipeId !== nextCard.recipeId) return false;
  if (prevCard.rotate !== nextCard.rotate) return false;
  if (prevCard.zIndex !== nextCard.zIndex) return false;
  if (prevCard.pinColor !== nextCard.pinColor) return false;
  if (prevCard.pinTop !== nextCard.pinTop) return false;
  if (prevCard.pinLeft !== nextCard.pinLeft) return false;
  if (prevCard.title !== nextCard.title) return false;
  if (prevCard.category !== nextCard.category) return false;
  if (prevCard.time !== nextCard.time) return false;
  if (prevCard.text !== nextCard.text) return false;
  if (prevCard.image !== nextCard.image) return false;
  if (prevCard.imageAlt !== nextCard.imageAlt) return false;
  if (prevCard.ingredients !== nextCard.ingredients) return false;
  if (prevCard.instructions !== nextCard.instructions) return false;
